...
fn build_blacklist_executor<'a>(
    ...
    let mut executor = build_executor(db, receipt_builder, hardforks, 1_000_000, JOVIAN_TIMESTAMP);
...
fn it_query_revert_and_malformed_fail_open() {
        ...
        [BLACKLIST_REVERT, &[0x60, 0x01, 0x60, 0x00, 0x53, 0x60, 0x01, 0x60, 0x00, 0xf3][..]]
    {
        ...
        executor.execute_transaction(&recovered_deposit()).expect("query failure is fail-open");
        assert!(executor.receipts[0].status());
        assert_eq!(
...
fn it_add_then_deposit_intercepts() {
    ...
    executor.execute_transaction(&recovered_deposit()).expect("deposit remains includable");
    assert!(executor.receipts[0].status());
...
fn it_remove_then_deposit_executes() {
    ...
    executor.execute_transaction(&recovered_deposit()).expect("deposit executes");
    assert!(executor.receipts[0].status());
    assert!(executor.receipts[1].status());
}
...
fn it_deposit_then_later_add_not_retroactive() {
    ...
    executor.execute_transaction(&recovered_deposit()).expect("deposit executes first");
...
fn it_hit_then_normal_tx_isolated() {
    ...
    side_effect.execute_transaction(&recovered_deposit()).expect("failed deposit included");
    ...
    control.execute_transaction(&recovered_deposit()).expect("control failed deposit included");
...
fn it_query_state_is_never_committed() {
    ...
    let control_gas =
        control.execute_transaction(&recovered_deposit()).expect("control deposit included");
