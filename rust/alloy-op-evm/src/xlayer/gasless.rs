...
mod xlayer_tests {
    ...
    fn test_gasless_fee_cfg_bypasses_fee_validation() {
        ...
        let mut evm = OpEvmFactory::<OpTransaction<TxEnv>>::default()
            .create_evm(EmptyDB::default(), env.clone());
        assert!(evm.transact(tx.clone()).is_err());

        let mut evm =
            OpEvmFactory::<OpTransaction<TxEnv>>::default().create_evm(EmptyDB::default(), env);
        assert!(
    ...
    fn test_gasless_fee_cfg_keeps_block_gas_limit_validation() {
        ...
        assert!(
            XLayerGaslessFeeHook::transact_with_gasless_fee_checks(&mut evm, tx, true).is_err()
        );
        assert!(!evm.ctx().cfg.disable_base_fee);
    }
}
