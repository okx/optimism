use crate::{OpEvm, OpEvmContext, OpEvmFactory, post_exec::PostExecEvmAdapter};
...
impl<E, F, DB, I> GaslessFeeHook<PostExecEvmAdapter<E, F, DB, I>> for XLayerGaslessFeeHook
where
    Self: GaslessFeeHook<E>,
    E: Evm,
    PostExecEvmAdapter<E, F, DB, I>: Evm<
        DB = E::DB,
        Tx = E::Tx,
        Error = E::Error,
...
{
    fn disable_gasless_fee_checks(
        evm: &mut PostExecEvmAdapter<E, F, DB, I>,
        disabled: bool,
    ) -> Option<OpFeeCheckState> {
        <Self as GaslessFeeHook<E>>::disable_gasless_fee_checks(&mut **evm, disabled)
    }
    ...
    fn restore_gasless_fee_checks(
        ...
        <Self as GaslessFeeHook<E>>::restore_gasless_fee_checks(&mut **evm, previous);
    }
}

/// No-op gasless fee hook for EVM types that do not need base-fee cfg manipulation.
///
/// Used for the [`PostExecEvmFactoryAdapter`] path where the concrete EVM is not known at
/// the factory impl level. Gasless fee logic in the handler still fires correctly because
/// `tx.is_gasless()` guards it; this hook simply skips the `disable_base_fee` toggle.
#[derive(Clone, Copy, Debug, Default)]
pub struct NullGaslessFeeHook;

impl<E: Evm> GaslessFeeHook<E> for NullGaslessFeeHook {
