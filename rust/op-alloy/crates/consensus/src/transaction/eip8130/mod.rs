...
// NOTE: tests.rs is currently disabled — base's upstream tests reference
// non-existent benches/fixtures/*.hex and use stale `from: Address` literals
// that don't match the current `from: Option<Address>` field type. We will
// re-introduce verification tests in a follow-up after the production code
// compiles cleanly.
// #[cfg(test)]
// mod tests;
