    ...
    TxEnvFor<N::Evm>: OpTxTr + OpTxEnv,
{
    ...
    /// then runs through `OpEvm::transact_raw`, which disables base-fee validation for it without
    /// changing the opcode-visible base fee. There is no inspector here, so detection cannot
