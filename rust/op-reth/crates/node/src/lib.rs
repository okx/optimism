...
// Adding the EIP-8130 envelope variant + the OpHardfork::XLayerV1 variant + xlayer_v1
// genesis fields widened the transitive type-graph used by `BasicPayloadJob<...>`'s
// `Unpin` auto-trait walk past rustc 1.94's default `recursion_limit = 128`. The
// obligation itself terminates fine (auto-traits stop at concrete types) — the depth
// of the type-tree walk is what needs more headroom. Bumped to 512 because 256 was
// just barely over after the XLayerV1 fork plumbing landed.
#![recursion_limit = "512"]
