...
mod tests {
    ...
    struct VecBest {
        txs: VecDeque<Arc<ValidPoolTransaction<OpPooledTransaction>>>,
    }

    impl VecBest {
        fn new(txs: Vec<Arc<ValidPoolTransaction<OpPooledTransaction>>>) -> Self {
            Self { txs: txs.into() }
        }
    }

    impl Iterator for VecBest {
        type Item = Arc<ValidPoolTransaction<OpPooledTransaction>>;
        fn next(&mut self) -> Option<Self::Item> {
            self.txs.pop_front()
        }
    }
