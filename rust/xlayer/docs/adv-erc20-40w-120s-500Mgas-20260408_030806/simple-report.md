# XLayer CL — Sequencer Performance Report
...
| CL | TPS | Block fill | Block Build Initiation Latency 99th Percentile | Maximum |
|---|---|---|---|---|
...
> **Block Build Initiation Latency (T1→T3)** = Block Build Initiation Request - Queue Dispatch Latency (T1→T2) + Block Build Initiation Request - Http Sender Roundtrip Latency (T2→T3). Lower is better.
...
|---|---|---|---|---|
| **BlockBuildInitiation-RequestGenerationLatency** (T0→T1) — Block Build Initiation Request Generation Latency | 247 ms | 104 ms | **100 ms** ✅ | 105 ms |
| **BlockBuildInitiation-QueueDispatchLatency** (T1→T2) — Block Build Initiation Request - Queue Dispatch Latency | N/A ¹ | 0.1 ms | 0.1 ms | 0.1 ms |
| **BlockBuildInitiation-HttpSender-RoundtripLatency** (T2→T3) — Block Build Initiation Request - Http Sender Roundtrip Latency | 2.2 ms | **2.0 ms** ✅ | 2.1 ms | **2.0 ms** ✅ |
| **BlockBuildInitiation-Latency** (T1→T3) — Block Build Initiation Latency | N/A ¹ | **2.2 ms** ✅ | **2.2 ms** ✅ | 2.3 ms |
| **T0→T3** — full block-build cycle | 249 ms | 111 ms | **102 ms** ✅ | 109 ms |
...
| Interval | op-node | kona baseline | **kona optimised** | base-cl |
|---|---|---|---|---|
| **BlockBuildInitiation-RequestGenerationLatency** (T0→T1) — Block Build Initiation Request Generation Latency | 230 ms | **102 ms** ✅ | 103 ms | 107 ms |
| **BlockBuildInitiation-QueueDispatchLatency** (T1→T2) — Block Build Initiation Request - Queue Dispatch Latency | N/A ¹ | 7.6 ms | **4.6 ms** ✅ | 5.6 ms |
| **BlockBuildInitiation-HttpSender-RoundtripLatency** (T2→T3) — Block Build Initiation Request - Http Sender Roundtrip Latency | 5.5 ms | **3.0 ms** ✅ | **3.0 ms** ✅ | **3.0 ms** ✅ |
| **BlockBuildInitiation-Latency** (T1→T3) — Block Build Initiation Latency | N/A ¹ | 11 ms | **7.6 ms** ✅ | 8.6 ms |
| **T0→T3** — full block-build cycle | 236 ms | 113 ms | **111 ms** ✅ | 116 ms |
...
| Interval | op-node | kona baseline | **kona optimised** | base-cl |
|---|---|---|---|---|
| **BlockBuildInitiation-RequestGenerationLatency** (T0→T1) — Block Build Initiation Request Generation Latency | 453 ms | **224 ms** ✅ | 264 ms | 326 ms |
| **BlockBuildInitiation-QueueDispatchLatency** (T1→T2) — Block Build Initiation Request - Queue Dispatch Latency | N/A ¹ | 111 ms | **98 ms** ✅ | 142 ms |
| **BlockBuildInitiation-HttpSender-RoundtripLatency** (T2→T3) — Block Build Initiation Request - Http Sender Roundtrip Latency | 111 ms | 37 ms | 31 ms | **19 ms** ✅ |
| **BlockBuildInitiation-Latency** (T1→T3) — Block Build Initiation Latency | N/A ¹ | 114 ms | **99 ms** ✅ | 152 ms |
| **T0→T3** — full block-build cycle | 455 ms | 298 ms | **270 ms** ✅ | 334 ms |
...
| Interval | op-node | kona baseline | **kona optimised** | base-cl |
|---|---|---|---|---|
| **BlockBuildInitiation-RequestGenerationLatency** (T0→T1) — Block Build Initiation Request Generation Latency | 522 ms | 281 ms | **273 ms** ✅ | 376 ms |
| **BlockBuildInitiation-QueueDispatchLatency** (T1→T2) — Block Build Initiation Request - Queue Dispatch Latency | N/A ¹ | 193 ms | **149 ms** ✅ | 151 ms |
| **BlockBuildInitiation-HttpSender-RoundtripLatency** (T2→T3) — Block Build Initiation Request - Http Sender Roundtrip Latency | 119 ms | 41 ms | 34 ms | **20 ms** ✅ |
| **BlockBuildInitiation-Latency** (T1→T3) — Block Build Initiation Latency | N/A ¹ | 194 ms | **151 ms** ✅ | 153 ms |
| **T0→T3** — full block-build cycle | 528 ms | 331 ms | **277 ms** ✅ | 394 ms |
...
|---|---|---|---|
| op-node | 230 ms | 453 ms | 522 ms |
| kona baseline | **102 ms** ✅ | **224 ms** ✅ | 281 ms |
| **kona optimised** | 103 ms | 264 ms | **273 ms** ✅ |
| base-cl | 107 ms | 326 ms | 376 ms |
...
flowchart LR
    subgraph FULL["◄──────── Block Build Initiation Latency  T1 → T3 ────────►"]
        ...
        T1 -->|"🟠 kona / base-cl: mpsc send\nbaseline up to 193 ms · optimised up to 149 ms"| CH
...
flowchart TB
    subgraph CLP["CL process  ←──────── Pre-HTTP Delay  T1→T2  lives entirely here ────────►"]
        ...
        T1 -->|"🟠 Block Build Initiation Request - Queue Dispatch Latency  T1→T2  (99th Percentile)\nkona: ① channel wait  ② drain → BinaryHeap  ③ priority pick\n─────────────────────────────────────────────\nop-node        ~0 ms ✅  no channel · no heap · direct RPC\nkona baseline  111 ms ❌  BlockBuildInitiationRequest starved behind AdvanceSafeHead in heap\nkona optimised 98 ms ✅  flush fix: BlockBuildInitiationRequest always wins heap\nbase-cl        142 ms     same heap pattern as kona baseline"| T2
    end
...
| kona baseline | BinaryHeap: `BlockBuildInitiationRequest` request waits behind `AdvanceSafeHead` tasks | 193 ms |
| **kona optimised** | `flush_pending_messages()` drains all pending tasks first → `BlockBuildInitiationRequest` always wins | **149 ms** |
| base-cl | Same BinaryHeap pattern as kona baseline, without the optimisation | 151 ms |
...
flowchart TB
    subgraph BEFORE["❌  Before — recv() one at a time"]
        ...
        HB -->|"dequeue top"| OB(["⏳  AdvanceSafeHead runs\n    BlockBuildInitiationRequest waits  up to 193 ms"])
...
- Pre-HTTP internal delay: 193 ms → 149 ms
...
**Use Maximum for the crisis headline.** "op-node's worst single block was 522 ms" — this answers "how bad can it get?" Even if Maximum is a rare event, it shows the risk envelope and motivates the fix.
...
| 99th Percentile (reliable tail) | 522ms range | Significantly lower — repeatable improvement |
| Maximum (worst block) | Spike driven by Go mutex + L1 RPC contention | Bounded — no shared mutex to contend |

---

*Full detailed report with all percentiles: `comparison.md` · Generated 2026-04-08*
