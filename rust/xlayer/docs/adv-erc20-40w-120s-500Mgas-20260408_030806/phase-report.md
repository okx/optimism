# XLayer CL — Block-Build Trigger: Phase-by-Phase Analysis
...
| **Accounts** | 50,000 pre-funded · same accounts across all CLs · sequential runs |
...
| 5 | **Block Build Initiation Latency (T1→T3)** | Primary benchmark metric. Time from the moment BlockBuildInitiationRequest is handed to the engine actor (T1) to the moment `payloadId` is received back from reth (T3) — i.e. the point where reth begins building the block. Covers Phase 3 + Phase 4. Also referred to as `FCU+attrs` in the OP Stack Engine API specification. |
...
| 11 | **sync.Mutex** | Go mutual-exclusion lock in op-node's `Driver` struct. Shared between the sequencer goroutine and the derivation goroutine — when one holds it, the other is blocked. Root cause of op-node's T0→T1 spikes under heavy L1 batch load. |
...
| 14 | **T2** · `BlockBuildInitiation-HTTPRequestSentTime` | **Phase 3 End Time / Phase 4 Start Time.** HTTP `engine_forkchoiceUpdated` request dispatched to reth — marks the end of Queue Dispatch Latency and the start of Http Sender Roundtrip Latency. For op-node this coincides with T1. |
| 15 | **T3** · `BlockBuild-ExecutionLayer-JobIdReceivedAt` | **Phase 4 End Time.** `payloadId` received back from reth — marks the end of Http Sender Roundtrip Latency and confirms reth has started building the block. This is the finish line for Block Build Initiation Latency (T1→T3). |
...
graph TD
    ...
    EA["🟡 EngineProcessor\nPriority BinaryHeap + drain loop\nSole owner of reth connection"]
    ...
    SA -->|"Build(PayloadAttributes)\nvia mpsc  ← T1 fires here"| EA
    SA -->|"Seal(payload_id)\nvia mpsc"| EA

    NA -->|"Insert(unsafe_block)\nvia mpsc"| EA
    RA -->|"pass-through\nvia mpsc"| EA

    EA -->|"engine_forkchoiceUpdatedV3 + attrs\n(Build — starts block in reth)"| RETH
    EA -->|"engine_forkchoiceUpdatedV3 no-attrs\n(Seal / Consolidate / Finalize)"| RETH
    EA -->|"engine_newPayloadV3\n(Seal / Insert)"| RETH
    EA -->|"engine_getPayloadV3\n(Seal)"| RETH
...
flowchart LR
    subgraph CYCLE["◄──────────────── Full Cycle  T0 → T3 ────────────────►"]
        ...
        ATTRBOX["① Block Build Initiation Request Generation Latency  T0→T1  ·  BlockBuildInitiation-RequestGenerationLatency\nL1 origin lookup · SystemConfig (L2 RPC) · L1 block data (L1 RPC)\nPayloadAttributes assembly"]
...
### T1→T3 — Block Build Initiation Latency *(kona fix signal)*

> Time from PayloadAttributes ready (T1) to payloadId received (T3). Covers queue wait + HTTP round-trip.
> This is the primary metric showing the kona optimisation effect — where `flush_pending_messages()` acts.
> op-node has no queue (T1≈T2), so its T1→T3 = T2→T3 = HTTP only — see Full Cycle below for the honest comparison.

| CL | TPS | Block fill | T1→T3 p99 | T1→T3 max |
|---|---|---|---|---|
| op-node | 11,249 TX/s | 79% | 111 ms ⁴ | 119 ms |
| kona baseline | **12,055 TX/s** | **84%** | 114 ms | 194 ms |
| kona optimised | 11,682 TX/s | 82% | **99 ms** ✅ | 151 ms |
| base-cl | 11,600 TX/s | 81% | 152 ms | 153 ms |

> ⁴ op-node has no queue — T1≈T2, so T1→T3 = HTTP only. The 453ms attr prep paid at T0→T1 is invisible here. See Full Cycle table.

---

### T0→T3 — Full Build Cycle *(honest cross-CL comparison)*

> T0→T3 = T0→T1 (attr prep) + T1→T3 (engine dispatch). Always use this when comparing op-node against kona.
> op-node prepares attributes synchronously — the entire node is frozen for the full T0→T1 duration.

| Interval | op-node | kona baseline | kona optimised | base-cl |
|---|---|---|---|---|
| T0→T1  Attr Prep · Median | 247 ms | 104 ms | **100 ms** ✅ | 105 ms |
| T0→T1  Attr Prep · p99 | **453 ms** ❌ | **224 ms** ✅ | 264 ms | 326 ms |
| T0→T1  Attr Prep · Max | **522 ms** ❌ | 281 ms | **273 ms** ✅ | 376 ms |
|   |   |   |   |   |
| T1→T3  Engine Dispatch · Median | N/A ¹ | **2.2 ms** ✅ | **2.2 ms** ✅ | 2.3 ms |
| T1→T3  Engine Dispatch · p99 | N/A ¹ | 114 ms | **99 ms** ✅ | 152 ms |
| T1→T3  Engine Dispatch · Max | N/A ¹ | 194 ms | **151 ms** ✅ | 153 ms |
|   |   |   |   |   |
| **T0→T3  Full Cycle · Median** | **249 ms** | 111 ms | **102 ms** ✅ | 109 ms |
| **T0→T3  Full Cycle · p99** | **455 ms** ❌ | 298 ms | **270 ms** ✅ | 334 ms |
| **T0→T3  Full Cycle · Max** | **528 ms** ❌ | 331 ms | **277 ms** ✅ | 394 ms |

> ¹ op-node has no queue (T1≈T2), so T1→T3 = T2→T3 = HTTP only = 2.2ms median · 111ms p99. Use T0→T3 to compare op-node against kona.

### Why op-node T1→T3 looks deceptively low

op-node's T1→T3 = 111ms p99 looks competitive — but T1 and T2 are the same moment (no queue). The 453ms of attribute prep at T0→T1 is hidden from the T1→T3 metric because T1 only starts *after* attrs are ready:

```text
op-node per block:

T0 ──[453ms p99 attr prep]──► T1 ──[~0ms no queue]──► T2 ──[111ms p99 HTTP]──► T3
     ← HIDDEN from T1→T3 →         T1≈T2                  ← T1→T3 measures →

T0→T3 full cycle = 455ms p99   ← honest number
kona-optimised   full cycle = 270ms p99   ← 1.7× faster at p99
```

The node is frozen for the entire T0→T3 window. kona makes the same L1 RPC calls but in a separate async task — derivation keeps running, T0→T3 median drops from 249ms to 102ms (2.4×).

---

## Phase 1 — Block Build Initiation Request Generation Latency  `T0 → T1`  ·  `BlockBuildInitiation-RequestGenerationLatency`
...
> **Why it matters:** op-node freezes the entire node during T0→T1; kona/base-cl run it concurrently.
...
flowchart TB
    subgraph OPNODE["⚠️  op-node — SYNCHRONOUS  (sync.Mutex held · Driver goroutine blocked · entire node frozen)"]
        ...
        OP1(["📍 T1\nBlockBuildInitiation-RequestGeneratedAt\nattrs ready\nT0→T1 duration: Median=247ms  99th Pct=453ms  Max=522ms\n⚠️ NOT the headline metric — see Results above"])
    ...
    subgraph KONA["✅  kona / base-cl — ASYNC  (independent Tokio task · derivation runs in parallel)"]
        ...
        KO1(["📍 T1\nBlockBuildInitiation-RequestGeneratedAt\nattrs ready\nkona:    Median=104ms  99th Percentile=224ms  Maximum=281ms\nbase-cl: Median=105ms  99th Percentile=326ms  Maximum=376ms"])
...
The `Build` task carries a full FCU+attrs call to reth. Every field in `payloadAttributes` has a specific source — this is the complete work of T0→T1:
...
| `gasLimit` | L2 RPC ③ | `SystemConfigByL2Hash(l2_parent_hash)` |
...
// At 500M gas: heavy L1 batches → AttemptStep() holds longer
// → sequencer tick queues → when it fires, RPC server already busy
// → PreparePayloadAttributes() spikes → T0→T1 reaches 522ms
```
...
T1 ──── BlockBuildInitiationRequest ready
         ...
         │   }
         │   T1→T2 ≈ 0 ms by architectural design (no queue exists)
         │
         └─ kona / base-cl ── mpsc CHANNEL — non-blocking send to engine actor
             ...
             │   //     so build_request_start.elapsed() at T3 = T1→T3 total
             │
...
## Phase 3 — Block Build Initiation Request - Queue Dispatch Latency  `T1 → T2`  ·  `BlockBuildInitiation-QueueDispatchLatency`  *(kona / base-cl only)*
...
flowchart TB
    ...
    subgraph AFTER["✅  kona optimised — flush first · BlockBuildInitiationRequest always wins"]
        ...
        DA --> OA(["📍 T2\nBlockBuildInitiation-HTTPRequestSentTime\nHTTP Roundtrip starts\n99th Percentile=98ms  Maximum=149ms"])
    end
...
## Phase 4 — Block Build Initiation Request - Http Sender Roundtrip Latency  `T2 → T3`  ·  `BlockBuildInitiation-HttpSender-RoundtripLatency`
...
> **Not to be confused with:** Block Build Initiation Latency (T1→T3) = Phase 3 + Phase 4 combined — the full interval from build decision to payloadId. Block Build Initiation Request - Http Sender Roundtrip Latency (T2→T3) is only the wire call portion.
...
T2→T3 is the same at Median for all CLs. 99th Percentile / Maximum divergence reflects reth state under load, not CL behaviour.

---

## kona Optimisation Summary

- **1.2×** lower Block Build Initiation Latency 99th Percentile  (114 ms → 99 ms vs kona baseline)
- **1.3×** lower Block Build Initiation Latency Maximum  (194 ms → 151 ms vs kona baseline)
- Pre-HTTP queue stall eliminated: 193 ms → 149 ms (max)
- Same throughput as op-node — zero regression in TPS or block fill
...
*Full percentile data: `comparison.md` · Generated 2026-04-08*
