...
fn verify(attestation: &[u8], commit_slice: impl Fn(&[u8])) {
    ...
    let pcr0 = &attestation[offset + 3..offset + 51];
    println!("PCR0: {:?}", pcr0);
    commit_slice(&Sha256::digest(pcr0));

    // skip PCR1 through PCR15
    offset += 51;
    ...
    // commit public key
    let pubkey_len = if attestation[offset] == 0xf6 {
        // CBOR null - no public key
        offset += 1;
        0
    } else if attestation[offset] >= 0x40 && attestation[offset] <= 0x57 {
...
mod tests {
    ...
    fn test_aws() {
        ...
        // real Nitro enclave
        let attestation =
            std::fs::read(concat!(env!("CARGO_MANIFEST_DIR"), "/src/testcases/aws.bin"))
                .unwrap();
        ...
        let expected_journal = [
            // timestamp
            "00000193bef3f3b0",
            // pcr hash = SHA256(PCR0)
            "ce70ee3119006e410084d8d82527fb9fced62b917956880c494f257f0bfc6449",
            // root pubkey
    ...
    fn test_custom() {
        ...
        let attestation =
            std::fs::read(concat!(env!("CARGO_MANIFEST_DIR"), "/src/testcases/custom.bin"))
                .unwrap();
    ...
    fn test_aws_pcr16() {
        ...
        // real Nitro enclave
        let attestation =
            std::fs::read(concat!(env!("CARGO_MANIFEST_DIR"), "/src/testcases/aws_pcr16.bin"))
                .unwrap();
        ...
        let expected_journal = [
            ...
            "00000196811b1c0b",
            // pcr hash = SHA256(PCR0)
            "8dba4fc1771a50a5e2516072162526e3230f06e68e113b5b2cd387f06f787a5e",
            // root pubkey
            "fc0254eba608c1f36870e29ada90be46383292736e894bfff672d989444b5051e534a4b1f6dbe3c0bc581a32b7b17607",
            "0ede12d69a3fea211b66e752cf7dd1dd095f6f1370f4170843d9dc100121e4cf63012809664487c9796284304dc53ff4",
            // pubkey len
    ...
    #[test]
    fn test_aws_null_pubkey() {
        // attestation from our Nitro enclave with public_key: null
        let attestation =
            std::fs::read(concat!(env!("CARGO_MANIFEST_DIR"), "/src/testcases/aws_null_pubkey.bin"))
                .unwrap();
        ...
        let expected_journal = [
            ...
            "0000019cb7e42b00",
            // pcr hash = SHA256(PCR0)
            "67b3c3366cf4c21adaea4e8163c5f0d460c2b932f1f9355f0b9ab22ae9ff1f85",
            // root pubkey
            "fc0254eba608c1f36870e29ada90be46383292736e894bfff672d989444b5051e534a4b1f6dbe3c0bc581a32b7b17607",
            "0ede12d69a3fea211b66e752cf7dd1dd095f6f1370f4170843d9dc100121e4cf63012809664487c9796284304dc53ff4",
            // pubkey len (0 = null)
            "00",
            // no pubkey bytes
            // user data len (32 bytes = 0x0020)
            "0020",
            // user data
            "176646ebb4e3a465f59b93bd455f4c85111c7ede076ea3b4c0aba6670824376a",
        ].join("");
    ...
    fn test_custom_pcr16() {
        ...
        // on a custom mock attestation server running locally
        let attestation =
            std::fs::read(concat!(env!("CARGO_MANIFEST_DIR"), "/src/testcases/custom_pcr16.bin"))
                .unwrap();
