 ...
 *
 * Wire layout (must match `op_alloy_consensus::TxEip8130::encode_for_sender_signing`
 * exactly — see `rust/op-alloy/crates/consensus/src/transaction/xlayer.rs`):
 *
 ...
 *   • Fee model: EIP-1559-style with both `max_priority_fee_per_gas` and
 *     `max_fee_per_gas` on the wire — matches the Rust struct shape.
 *
...
const PRIV = (process.env.PRIV ??
    "0xd9fb56b9574ed61ab0478a607166eeb3a80b1b91ab1bf00f45932105d07b5e11") as Hex;
...
// EIP-1559-style fee. `MAX_FEE` is kept as a convenience alias for
// `MAX_FEE_PER_GAS`. `MAX_PRIORITY_FEE_PER_GAS` defaults to 0 (full-tip-to-base).
const MAX_FEE_PER_GAS = BigInt(
    process.env.MAX_FEE_PER_GAS ?? process.env.MAX_FEE ?? process.env.GAS_PRICE ?? "1000000000",
);
const MAX_PRIORITY_FEE_PER_GAS = BigInt(
...
async function main() {
    ...
    console.log(`max_fee    : ${MAX_FEE_PER_GAS}`);
    console.log(`max_prio   : ${MAX_PRIORITY_FEE_PER_GAS}`);
    ...
    // `TxEip8130::encode_for_sender_signing` exactly: EIP-1559-style with
    // `max_priority_fee_per_gas` followed by `max_fee_per_gas` between
    // `expiry` and `gas_limit`.
    const preimageFields: RlpNode = [
        ...
        uintToRlpHex(EXPIRY),
        uintToRlpHex(MAX_PRIORITY_FEE_PER_GAS),
        uintToRlpHex(MAX_FEE_PER_GAS),
