#!/bin/bash
# =============================================================================
# Step 50: Setup Fraud Proof System (Complete Flow from pre-release)
# Function: Exact copy of fraud proof logic from pre-release 4-op-start-service.sh
# =============================================================================
set -e
set -x
...
echo "=========================================="
echo "Step 50: Setup Fraud Proof (pre-release flow)"
echo "=========================================="
...
if [ ! -f "$EXPORT_DIR/prestate-proof-mt64.json" ]; then
    echo "ERROR: prestate file not found"
    exit 1
fi
...
"$STEPS_DIR/add-game-type.sh" 1 true $TEMP_CLOCK_EXTENSION $TEMP_MAX_CLOCK_DURATION $ABSOLUTE_PRESTATE
...
"$STEPS_DIR/add-game-type.sh" 0 false $CLOCK_EXTENSION $MAX_CLOCK_DURATION $ABSOLUTE_PRESTATE

export GAME_TYPE=0

sleep $TEMP_GAME_WINDOW
${DOCKER_COMPOSE_CMD} up -d --remove-orphans op-proposer op-challenger op-dispute-mon

echo ""
echo "SUCCESS: Step 50 completed - Fraud Proof system fully setup"
