...
docker compose -f ${DOCKER_COMPOSE_FILE} up -d op-batcher

sleep 10
# TODO, we need to reseach and fix it,  0 block hash mismatch
LOG_OUTPUT=$(docker compose -f ${DOCKER_COMPOSE_FILE} logs op-seq 2>&1 | tail -20)
if echo "$LOG_OUTPUT" | grep -q "expected L2 genesis hash to match L2 block at genesis block number"; then
    ...
    if [ -n "$CORRECT_HASH" ]; then
        ...
        docker compose -f ${DOCKER_COMPOSE_FILE} restart op-seq
    fi
fi
...
docker compose -f ${DOCKER_COMPOSE_FILE} up -d op-rpc
...
export GAME_TYPE=1
docker compose -f ${DOCKER_COMPOSE_FILE} up -d op-proposer
...
docker compose -f ${DOCKER_COMPOSE_FILE} stop op-proposer
...
docker compose -f ${DOCKER_COMPOSE_FILE} up -d op-proposer op-challenger op-dispute-mon
