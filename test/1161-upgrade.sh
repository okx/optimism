...
if [ ! -f "docker-compose.yml" ]; then
    log_error "docker-compose.yml not found. Are you in the test/ directory?"
    exit 1
fi
...
# Determine target version from current branch
CURRENT_BRANCH=$(cd .. && git rev-parse --abbrev-ref HEAD)
...
docker compose up -d "${SERVICES_TO_RESTART[@]}"
