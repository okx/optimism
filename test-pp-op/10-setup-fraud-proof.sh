...
set_game_type_permissionless() {
  ...
  # Call the function to add game type 0 (permissionless) via Transactor or Safe
  # Script is being executed directly - choose function based on OWNER_TYPE
  if [ "$OWNER_TYPE" = "safe" ]; then
      add_game_type_via_safe 0 false $CLOCK_EXTENSION $MAX_CLOCK_DURATION $ABSOLUTE_PRESTATE
