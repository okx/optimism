...
func VerifyEmbeddedCommit() error {
	...
	// X Layer: check (3) — cross-checking against op-geth's bundled superchain-registry
	// commit (gethsuperchain.EmbeddedRegistryCommit) — is intentionally omitted. okx/op-geth
	// is held at a fixed pin and does not expose that API; op-core/superchain is the single
	// source of truth for the embedded bundle in this fork.
	return nil
}
