...
// isLogsInitialized returns whether the logs DB for the given chain id is initialized.
func (db *ChainsDB) isLogsInitialized(id eth.ChainID) bool {
	ldb, ok := db.logDBs.Get(id)
	return ok && !ldb.IsEmpty()
}
...
func (db *ChainsDB) initCrossSafe(id eth.ChainID, derived types.DerivedBlockRefPair) error {
	...
	db.m.RecordCrossSafe(id, types.BlockSealFromRef(derived.Derived))
	return nil
}
...
func (db *ChainsDB) initLocalSafe(id eth.ChainID, derived types.DerivedBlockRefPair) error {
	...
	db.m.RecordLocalSafe(id, types.BlockSealFromRef(derived.Derived))
	return nil
}
