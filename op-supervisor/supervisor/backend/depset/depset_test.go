...
func TestDependencySet(t *testing.T) {
	...
	v, err := result.CanExecuteAt(eth.ChainIDFromUInt64(900), 43)
	require.NoError(t, err)
	require.True(t, v)
	v, err = result.CanExecuteAt(eth.ChainIDFromUInt64(900), 42)
	require.NoError(t, err)
	require.False(t, v)
	v, err = result.CanInitiateAt(eth.ChainIDFromUInt64(900), 101)
	require.NoError(t, err)
	require.True(t, v)
	...
	v, err = result.CanExecuteAt(eth.ChainIDFromUInt64(901), 31)
	require.NoError(t, err)
	require.True(t, v)
	v, err = result.CanExecuteAt(eth.ChainIDFromUInt64(901), 30)
	require.NoError(t, err)
	require.False(t, v)
	v, err = result.CanInitiateAt(eth.ChainIDFromUInt64(901), 21)
	require.NoError(t, err)
	require.True(t, v)
